...
df = pd.read_excel('ww.xlsx',sheet_name='门急诊收入排名')
# 获取类别、数据和列名
categories = df.iloc[:, 0]
x = df.iloc[:, 1]#总收入x
y = df.iloc[:, 2]#//服务收入y
columns = df.columns
colors = np.random.rand(categories.size)
# 计算数据的平均值
meanx = np.mean(x)
meany = np.mean(y)
...
# 设置图形标题和轴标签
ax.set_title('2022年各科室门急诊收入波士顿矩阵图')
...
# 保存图像
plt.savefig('门急诊收入.jpg', dpi=1200, bbox_inches='tight')
